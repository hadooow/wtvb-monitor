...
def test_terminator_missing_recovers_instead_of_faulting_the_gateway(monkeypatch):
    gateway = SerialGateway('COM3', 115200)
    gateway._serial = FakeSerial()
    gateway._running.set()
    ticks = iter([0, 0, 100])
...
def test_empty_list_accepts_optional_ok_and_does_not_leak_to_next_command():
    ...
    assert gateway._desynced and not gateway._faulted  # Query's OK cannot acknowledge the silent OP query.
