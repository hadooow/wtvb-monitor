...
class Scheduler:
    ...
    async def _handle_event(self, event: GatewayEvent) -> None:
        ...
        if event.kind == "scan":
            ...
            state.status = (
                "paused" if state.manual_paused
                else "retrying" if state.recovery_reason else "queued"
            )
            state.recovery_reason = None
            state.disconnect_requested = False
            state.connected_at = None
    ...
    def _schedule_serial(self) -> None:
        ...
        for mac in tuple(gateway.active_links):
            state = self.states.get(mac)
            if mac not in self._device_configs or (state is not None and state.manual_paused):
        ...
        if not self._serial_draining and not focus_switch and any(
            ...
            return
        if not self._serial_draining and not focus_switch and len(active) < limit:
    ...
    async def pause_device(self, mac: str) -> None:
        ...
        if state.status == "disconnecting":
            # The scheduler already queued this exact disconnect for a batch
    ...
    async def resume_device(self, mac: str) -> None:
        mac = normalize_mac(mac)
        state = self.states.get(mac)
        if state is None:
