...
class Database:
    ...
    def history_range(
        self, device_id: int, start: str, end: str, max_points: int | None = 2000
    ) -> list[dict[str, Any]]:
        """Return chronologically ordered history, evenly sampled for charts."""
        max_points = min(max(int(max_points), 1), 5000) if max_points is not None else None
        with self._lock, self.connect() as connection:
            if max_points is None:
                rows = connection.execute(
                    "SELECT * FROM samples WHERE device_id=? AND timestamp>=? AND timestamp<=? "
                    "ORDER BY timestamp ASC",
                    (device_id, start, end),
                ).fetchall()
                return [dict(row) for row in rows]
            count = connection.execute(
                "SELECT COUNT(*) FROM samples WHERE device_id=? AND timestamp>=? AND timestamp<=?",
                (device_id, start, end),
            ).fetchone()[0]
            if not count:
                return []
            ...
            rows = connection.execute(
                """
                SELECT * FROM (
                    SELECT samples.*, row_number() OVER (ORDER BY timestamp ASC) AS sample_no,
                           count(*) OVER () AS sample_count
                    FROM samples
                    WHERE device_id=? AND timestamp>=? AND timestamp<=?
                )
                WHERE (sample_no - 1) % ? = 0 OR sample_no=sample_count
                ORDER BY timestamp ASC
                """,
                (device_id, start, end, stride),
            ).fetchall()
        return [{key: row[key] for key in row.keys() if key not in {"sample_no", "sample_count"}} for row in rows]

    def history_page(
        self, device_id: int, start: str, end: str, page: int = 1, page_size: int = 100
        ...
        with self._lock, self.connect() as connection:
            ...
            rows = connection.execute(
                ...
                (device_id, start, end, page_size, offset),
            ).fetchall()
