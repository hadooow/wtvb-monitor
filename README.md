...
2. 在 v0.6.7 的 Assets 中下载 `WTVB-Monitor-v0.6.7-windows-x64.zip`。GitHub 自动提供的 `Source code` 是源码，不能直接当作 EXE 运行。
3. 右键 ZIP → 全部解压，建议解压到有写入权限的目录，例如 `D:\WTVB-v0.6.7`。
...
- **最方便：** 首页点击“下载诊断日志”，浏览器下载 `WTVB-diagnostics-v0.6.7.zip`。其中含 `diagnostics.json`（版本、配置及状态快照）和 `logs/monitor.log*`。
...
脚本先运行测试，再用 PyInstaller 打包，复制静态资源及现场配置，在临时副本中自动运行 EXE 验证模拟采集、首页及日志下载。成功后生成 `release\WTVB-Monitor-v0.6.7-windows-x64.zip` 和 SHA256 文件。临时自检数据不会装进发布包。
