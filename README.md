...
修复 v0.6.5 设置两路却只连接一台、优先设备长时间排队的问题。串口模式先用同一轮扫描收集候选设备，确认停扫后逐台建立连接，已连接设备同时持续回传；第二台不再受“必须正在扫描”和移动的 10 秒发现期限限制。

优先切换到已连接设备时保留两路采集；目标不在当前候选批次时主动释放当前批次并重新发现。发现窗口至少 3 秒，设备不足或优先目标未出现时等待至 8 秒再选择已发现设备；没有候选时继续扫描。通道未满时最多每 30 秒重建发现批次，避免未发现设备让其他设备频繁断开。重开软件可接管多路已存在的有效通知连接。

串口发送尽量等待完整行后的空闲窗口。停扫、启扫、查询以及按 MAC 断开等重复执行不会增加连接的操作，在无完整回复时最多重发两次；全过程仍由同一事务接收回复。连接命令和任意写配置命令不盲目重发，无法确认时保留首个阻断错误并暂停队列。

损坏或不支持格式的通知被丢弃并单独提示，不再把设备误设为连接失败；保留 v0.6.5 的坏通知、非 ASCII 字节计数。软件能够减少并容忍部分异常，但现场仍观测到少量串口数据损坏，不能据此声称物理链路已完全恢复。详见 [版本说明](RELEASE_NOTES.md)。
...
2. 在 v0.6.6 的 Assets 中下载 `WTVB-Monitor-v0.6.6-windows-x64.zip`。GitHub 自动提供的 `Source code` 是源码，不能直接当作 EXE 运行。
3. 右键 ZIP → 全部解压，建议解压到有写入权限的目录，例如 `D:\WTVB-v0.6.6`。
...
- **最方便：** 首页点击“下载诊断日志”，浏览器下载 `WTVB-diagnostics-v0.6.6.zip`。其中含 `diagnostics.json`（版本、配置及状态快照）和 `logs/monitor.log*`。
...
脚本先运行测试，再用 PyInstaller 打包，复制静态资源及现场配置，在临时副本中自动运行 EXE 验证模拟采集、首页及日志下载。成功后生成 `release\WTVB-Monitor-v0.6.6-windows-x64.zip` 和 SHA256 文件。临时自检数据不会装进发布包。
